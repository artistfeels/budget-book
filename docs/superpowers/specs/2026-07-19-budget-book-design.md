...
- **알려진 한계**: 매칭은 입력 순서대로 각 후보의 최근접 짝을 그리디로 찾는 방식이라, "전역 최근접 쌍 우선" 방식과 이론적으로 다른 결과를 낼 수 있다 (예: 같은 계좌에서 동일 금액 출금이 수 초 간격으로 여러 건 있고 입금 후보가 하나뿐인 경우). 실제 샘플 데이터(140건)에서는 이런 충돌이 없었고, 미매칭 건은 어떤 경우든 조용히 버려지지 않고 경고로 노출되므로 사용자 확인으로 보정 가능 — 2026-07-19 구현 시점에 현재 방식을 그대로 유지하기로 결정함.
